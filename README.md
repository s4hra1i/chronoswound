...
The planned panel-only sensitivity analysis included the sample excluded for missing age, giving
50 samples from 40 patients. Panel MAE was 3.00 days versus 4.41 days for the training-fold
median. This sensitivity result had no separate success criterion and does not replace the
registered 49-sample primary estimand.

Performance was not uniform across the observed range. The sole 27-day sample was predicted at
approximately 13.8 days after averaging its out-of-fold predictions, and samples at 19 and 22
days also had errors above eight days. The reported uncertainty concerns cohort-level
performance and paired model comparisons; the primary analysis does not provide calibrated
prediction intervals for individual wound-age estimates.

![Panel predictions across the observed range](reports/gse178411/figures/panel_predicted_vs_observed.png)
...
For exact reproduction of the archived v0.3.1 GSE178411 outputs under Python 3.12, install the
pinned scientific environment before installing the local package:
...
An algorithmic estimate could influence legal decisions and must never be presented without its uncertainty, provenance and limitations. Performance averages can conceal systematic error across ancestry, age, health status or tissue type. ChronosWound therefore makes grouped evaluation and explicit uncertainty reporting defaults, while distinguishing population-level performance uncertainty from individual prediction intervals.
